...
TIMEOUT = env.int('TIMEOUT', 60000)

SIDECAR_WAIT_TIME = env.int('SIDECAR_WAIT_TIME', 240)
...
class TestingConfig(DefaultConfig):
    DB_URI = env.str('DB_TEST_URI', DB_URI)
    CELERY_APP_CONFIG_FILE = env.str('CELERY_APP_TEST_CONFIG_FILE', 'config_testing.py')
    CELERY_BROKER_URL = env.str('CELERY_BROKER_TEST_URL', CELERY_BROKER_URL)
    CELERY_BACKEND_URL = env.str('CELERY_BACKEND_TEST_URL', CELERY_BACKEND_URL)
